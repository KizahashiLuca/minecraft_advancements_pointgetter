...
tellraw @a ["",{"translate":"     >> %s","with":[{"text":"トライアル中断","color":"red","bold":true,"hoverEvent":{"action":"show_text","value":"クリックしてトライアル中断"},"clickEvent":{"action":"run_command","value":"/function map:stop"}}],"color":"white","bold":false,"italic":false}]
